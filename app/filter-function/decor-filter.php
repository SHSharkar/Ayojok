...
if (isset($experience) && empty($event_type) && empty($city) && empty($price)) {
...
elseif (isset($experience) && isset($event_type) && empty($city) && empty($price)) {
...
elseif (isset($price) && empty($event_type) && empty($experience) && empty($city)) {
...
elseif (isset($price) && isset($event_type) && empty($experience) && empty($city)) {
...
elseif (isset($price) && isset($event_type) && isset($experience) && empty($city)) {
  ...
  if ($experience == ">20") {
    ...
    $explode = explode('-',$experience); $datas->where('event_type','like','%'.$event_type.'%')->whereBetween('feature_2',[$explode[0],$explode[1]])->where('low_price','>=',$price);
  }
  $datas = $datas->with('vendor')->get();
}
// If price, event_type and experience present filter
