...
elseif (isset($price) && isset($speciality) && isset($experience) && empty($city)) {
  ...
  if ($experience == ">20") {
    $datas = $datas->where('speciality_type','like','%'.$speciality.'%')->where('feature_2','>',20)->where('low_price','>=',$price);
  }
  else{
    $explode = explode('-',$experience);
    $datas = $datas->where('speciality_type','like','%'.$speciality.'%')->whereBetween('feature_2',[$explode[0],$explode[1]])->where('low_price','>=',$price);
  }
  $datas = $datas->with('vendor')->get();
}
// If price, speciality and experience present filter
