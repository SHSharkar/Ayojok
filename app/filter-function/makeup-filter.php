...
elseif (isset($home_service) && empty($experience) && empty($price)) {
  //echo "I am with home_service";
    $datas = $datas->where('feature_5','=',$home_service)->with('vendor')->get();
}
...
elseif (isset($experience) && isset($home_service) && empty($price)) {
  ...
  if ($experience == ">20") {
    //echo $home_service."+ 1-20";
    $datas = $datas->where('feature_5','=',$home_service)->where('feature_2','>',20);
  }
  else{
    $explode = explode('-',$experience);
    $datas = $datas->where('feature_5','=',$home_service)->whereBetween('feature_2',[$explode[0],$explode[1]]);
  }
...
elseif (isset($price) && isset($home_service) && empty($experience) && empty($city)) {
  //echo "If price, home_service present and experience,city null filter";
    $datas = $datas->where('low_price','>=',$price)->where('feature_5','=',$home_service)->with('vendor')->get();

}
// End, If price, home_service present and experience null filter

// If price, home_service and experience present filter
elseif (isset($price) && isset($home_service) && isset($experience) && empty($city)) {
  //echo "I am with price, home_service and experience";
  if ($experience == ">20") {
    $datas = $datas->where('feature_5','=',$home_service)->where('feature_2','>',20)->where('low_price','>=',$price);
  }
  else{
    $explode = explode('-',$experience);
    $datas = $datas->where('feature_5','=',$home_service)->whereBetween('feature_2',[$explode[0],$explode[1]])->where('low_price','>=',$price);
  }
  $datas = $datas->with('vendor')->get();
}
// If price, home_service and experience present filter
